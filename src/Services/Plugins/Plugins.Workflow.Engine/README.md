...
| `StuckStepRecoverySeconds` | 1 h | Crash backstop: `running` steps whose `updated_at` is older than this return to `pending` (attempt stays counted — MaxAttempts bounds crash loops). Must exceed the longest legitimate execution (lane budgets + drain); raise/disable if the long-lane timeout is disabled. 0 disables. |
